...
int main()
{
	...
	scanf("%d", &kind1);
	printf("请输入上货通道（有1，2，3，4，5共五个通道）：");
	scanf("%d", &road1);
...
setornot:
	...
	{
	case 1:
		printf("当前通道%d中有货物%c%d个，是否确认购买？（确认请输入1，否请输入0）",road2, goodsprint[kind2 - 1],goods1[kind2-1]);
		scanf("%d",&buyornot);
		if (buyornot == 0)
		{
			goto buy;
	...
	case 2:
		printf("当前通道%d中有货物%c%d个，是否确认购买？（确认请输入1，否请输入0）",road2, goodsprint[kind2-1], goods2[kind2 - 1]);
		scanf("%d",& buyornot);
		if (buyornot == 0)
		{
			goto buy;
	...
	case 3:
		printf("当前通道%d中有货物%c%d个，是否确认购买？（确认请输入1，否请输入0）", road2, goodsprint[kind2 - 1], goods3[kind2 - 1]);
		scanf("%d", &buyornot);
		if (buyornot == 0)
		{
			goto buy;
	...
	case 4:
		printf("当前通道%d中有货物%c%d个，是否确认购买？（确认请输入1，否请输入0）", road2, goodsprint[kind2 - 1], goods4[kind2 - 1]);
		scanf("%d", &buyornot);
		if (buyornot == 0)
		{
			goto buy;
	...
	case 5:
		printf("当前通道%d中有货物%c%d个，是否确认购买？（确认请输入1，否请输入0）", road2, goodsprint[kind2 - 1], goods5[kind2 - 1]);
		scanf("%d",& buyornot);
		if (buyornot == 0)
		{
			goto buy;
	...
	coinoperated:
		...
		{
			printf("投币中，请选择投币金额（每次仅可投入1，2，5元）\n（当前已投入%d元）:",sum);
			scanf("%d", &coin);
...
//这里上货采用的是多次上货，如果不加限制会出现同一类型货物不同价钱的情况，但那样不如直接一次性上货完毕，这会在改良版售货机实现